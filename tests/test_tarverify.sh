...
($TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/a" "$IDIR/b") && EC=0 || EC=$?
[ "$EC" -eq 3 ] || { echo "FAIL: expected exit 3 on first append, got $EC"; exit 1; }
$TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/a" "$IDIR/b"

[ -f "$TRASH$IDIR/a/f1.txt" ] || { echo "FAIL: f1 not trashed"; exit 1; }
[ -f "$TRASH$IDIR/b/f2.txt" ] || { echo "FAIL: f2 not trashed"; exit 1; }
[ -L "$TRASH$IDIR/a/link1.txt" ] || { echo "FAIL: link1 not trashed"; exit 1; }
...
($TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/a" "$IDIR/b") && EC=0 || EC=$?
[ "$EC" -eq 3 ] || { echo "FAIL: expected exit 3 on append, got $EC"; exit 1; }
$TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/a" "$IDIR/b"
...
# ===== test 4: changed content =====
echo "--- Test 4: changed content ---"
mkdir -p "$IDIR/c"

echo "v1" > "$IDIR/c/change.txt"
($TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/c") && EC=0 || EC=$?
[ "$EC" -eq 3 ] || { echo "FAIL: expected exit 3 on v1 append, got $EC"; exit 1; }
$TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/c"
[ -f "$TRASH$IDIR/c/change.txt" ] || { echo "FAIL: v1 not trashed"; exit 1; }

echo "v2" > "$IDIR/c/change.txt"
($TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/c") && EC=0 || EC=$?
[ "$EC" -eq 3 ] || { echo "FAIL: expected exit 3 on v2 append, got $EC"; exit 1; }
$TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/c"
...
($TARVERIFY -f "$TAR" -t "$TRASH" --timeout 0 "$IDIR/a") && EC=0 || EC=$?
[ "$EC" -eq 2 ] || { echo "FAIL: expected exit 2 from timeout, got $EC"; exit 1; }

($TARVERIFY -f "$TAR" -t "$TRASH" "$IDIR/a" "$IDIR/b") && EC=0 || EC=$?
[ "$EC" -eq 3 ] || { echo "FAIL: expected exit 3 from append, got $EC"; exit 1; }
